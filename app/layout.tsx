...
export const metadata: Metadata = {
  ...
  twitter: {
    ...
    title: '실거래마스터 - 아파트 실거래 정보',
    description:
      '전국 아파트 단지와 실거래 내역을 지역과 단지명으로 검색하고, 최근 거래와 가격 정보를 확인할 수 있습니다.',
  },
  robots: {
    index: true,
    follow: true,
    ...
    <header className="sticky top-0 z-40 border-b border-white/70 bg-[rgba(249,246,241,0.78)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 md:px-6 lg:px-8">
        <Link href="/" className="flex min-w-0 items-center gap-3">
          <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#1f3147] text-sm font-bold text-white shadow-[0_10px_30px_rgba(31,49,71,0.18)]">
            RM
          </span>
          <div className="min-w-0">
            <div className="truncate text-[15px] font-bold tracking-[-0.02em] text-[#1f3147]">
              실거래마스터
            </div>
            <div className="truncate text-xs text-[#7d7366]">
              아파트 실거래 정보 서비스
            </div>
          </div>
        </Link>
        ...
        <nav className="hidden items-center gap-2 md:flex">
          <Link
            href="/"
            className="rounded-full px-4 py-2 text-sm font-medium text-[#2b3647] transition hover:bg-white/80"
          >
            홈
          </Link>
        </nav>
...
export default function RootLayout({
  ...
  return (
    ...
    >
      <body className="flex min-h-full min-w-0 flex-col overflow-x-hidden bg-[#f7f2eb] text-gray-900">
        <Script
